...
The output is designed to be turned into a `mids` object with the
**mice** package and pooled:
...
- **Seed.** `seed` defaults to `NULL`, so imputations are random each
  run. Pass an integer for reproducibility.
- **Baseline.** Supply the baseline outcome as a covariate (`covar`)
  rather than as an outcome value, so that no treatment effect is
  assumed at baseline.
- **MCMC settings.** `burnin` controls the number of burn-in iterations
  when fitting the multivariate normal model; the default is generally
  adequate. Increase `M` for more stable pooled inference.
